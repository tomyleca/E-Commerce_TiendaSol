...
describe("Validar usuario", () => {
  ...
  beforeEach(() => {
    ...
    usuariosRepository.buscarPorId = jest.fn((id) =>
      _usuarios.find((u) => u.id === id),
    );
    ...
    productosRepository.buscarPorId = jest.fn((id) =>
      _productos.find((p) => p.id === id),
    );
    ...
    productoEnDolares = new Producto(
      vendedor,
    ...
    pedidosRepository.buscarPorId = jest.fn((id) =>
      _pedidos.find((p) => p.id === id),
    );
    pedidosRepository.actualizarEstado = jest.fn((pedido, estado) => {
      pedido.actualizarEstado(estado);
      return pedido;
    });
    pedidosRepository.buscarPorUsuario = jest.fn((idUsuario) =>
      _pedidos.filter((p) => p.comprador.id === idUsuario),
    );
    let categoriasRepository = new CategoriasRepository();
    let categoriasService = new CategoriaService(categoriasRepository);

    let productosService = new ProductosService(
      productosRepository,
      UsuariosService,
      categoriasService,
    );
