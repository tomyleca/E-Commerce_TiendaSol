...
export class ProductosService {
  ...
  async buscarTodosPaginado(pagina, cantidadPorPagina, querys) {
    ...
    const productos = await this.productosRepository.buscarTodos(
      filtros,
      ordenamiento,
      comienzo,
      cantidadPorPagina,
  ...
  async generarQueryFiltros(filtros) {
    ...
    if (
      !mongoose.Types.ObjectId.isValid(filtros.vendedor) &&
      filtros.vendedor
    ) {
  ...
  async crear(productoJson) {
    const vendedor = await this.usuariosService.buscarPorId(
    ...
    if (productoJson.categoriasId) {
      categorias = await Promise.all(
        productoJson.categoriasId.map(async (categoriaId) => {
          const categoria =
            await this.categoriasService.buscarPorId(categoriaId);
          if (!categoria) {
