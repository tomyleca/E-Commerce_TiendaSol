...
describe('Búsqueda y Filtrado de Productos', () => {
  ...
  it('debe mostrar productos visibles inicialmente', () => {
    cy.get(`${accederProducto} .product-title`)
      .should('have.length.at.least', 1); // al menos 1 producto
  ...
  it('debe filtrar productos al buscar por nombre', () => {
    ...
    cy.get(`${accederProducto} .product-title`)
      .should('have.length.at.least', 1); // que existan resultados

    cy.get(`${accederProducto} .product-header .product-title`).each($el => {
      cy.wrap($el)
        .invoke('text')
...
cy.get('.check-input').first().then($label => {
    ...
    cy.get(`${accederProducto} .categorias-card .categoria `).each($el => {
